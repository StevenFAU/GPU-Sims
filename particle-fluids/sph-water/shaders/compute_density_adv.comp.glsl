// compute_density_adv.comp.glsl — DFSPH density-advect source term.
//
// densityAdv_i = density_i / density0 + h * V_i * Σ_j (v_i − v_j) · ∇W_ij
//
// Result written to da[gid].z. Other components (.x density, .y α/ρ², .w
// density_change) are preserved.
//
// Reference: SPlisHSPlasH 2.16.1 SPlisHSPlasH/DFSPH/TimeStepDFSPH.cpp:1191-1242 (computeDensityAdv).
#version 460
...
void main() {
    ...
    // Fluid-only contribution scaled by V_i, mirroring
    // SPlisHSPlasH 2.16.1 SPlisHSPlasH/DFSPH/TimeStepDFSPH.cpp:1212 (delta *= model->getVolume(i)).
    float densityAdv = density_i / max(density0, 1e-7) + dt * V_i * delta;

    // Akinci2012 boundary contribution at SPlisHSPlasH 2.16.1 SPlisHSPlasH/DFSPH/TimeStepDFSPH.cpp:1217-1223:
    // boundary v_j = 0 for static rigid (current AABB), so contribution
    // reduces to V_b * vel_i · grad_W.
    if (boundaryParticleCount > 0u) {
        ...
        for (int dx = -1; dx <= 1; ++dx) {
            ...
            for (uint k = nstart; k < nend; ++k) {
                ...
                if (q >= 1.0 || r_mag < 1e-7) continue;
                vec3 grad_W = kernel_gradW(r_ij, r_mag, q, gradKernelNorm3D);
                delta_b += Vb_j * dot(vel_i, grad_W);
            }
        }
        densityAdv += dt * delta_b;
    }
