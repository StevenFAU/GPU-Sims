...
| 19 | **Runtime FFT-backend selection for Stack D sims that need FFT-class operations** | For sims where Taichi's lack of built-in FFT is load-bearing (lenia-fft consumer #1; future continuous-CA / spectral-methods sims may follow): ship multiple FFT-or-equivalent backends in priority order; probe each at init via guarded import + smoke test; log which backend won; fall back gracefully. Backends gated behind pip extras (`[cuda]`, `[rocm]`). Universal baseline (Taichi-real-space or numpy) always available. The exact priority order is sim-specific (lenia-fft: CuPy → PyTorch → Taichi-real-space → numpy); the *pattern* is the convention. See `continuous-ca/lenia-fft/docs/load-bearing-decisions.md`. | Phase 10 |
| 20 | **Sim-level 2D-vs-3D scope contingency banking** | For sims that may ship both a 2D load-bearing path and a 3D opt-in stretch path, the phase spec MUST document an explicit scope-cut fallback ("ship 2D-only as Phase N, bank 3D as Phase N.5") with specific file-manifest deltas. Prevents mid-execution scope creep AND lets architect-2 review recommend the scope cut cheaply if 3D adds disproportionate testing surface. 3D-cut deltas are localized: one tier in the dropdown, one preset, kernel functions, one export branch, one render script, one docs section; no `common-py` ripple, no CI ripple. See `continuous-ca/lenia-fft/docs/load-bearing-decisions.md`. | Phase 10 |
...
**Phase 10 promotion-review (consumer #2; documentation-only per rule-of-three).** Seven candidates inventoried in `continuous-ca/lenia-fft/docs/load-bearing-decisions.md` § "Promotion-review for `common-py` (consumer #2)". STRONG promote at consumer #3: `_cursor_in_any_panel` GUI-occlusion test, capture-mode confirmation modal, tier dropdown + deferred-change-after-`window.show()`. MODERATE: F5/F9 save-load buttons. KEEP sim-local: `set_color_by_material` re-apply, reserve-tail emitter allocation, `cursor_to_ground` 3D unproject. **No code extracted in Phase 10** — consumer #3 (likely `neural-ca` Stack D variant) triggers actual promotion work.
...
| `continuous-ca/lenia-fft/` | lenia-fft | D | **Implemented (Phase 10)** — first Stack D continuous-CA sim, second `common-py` consumer; runtime FFT-backend selection (CuPy / PyTorch / Taichi-real-space / numpy) with init-time priority probe; three 2D tiers + opt-in 3D stretch; four canonical creature presets verified byte-for-byte against `Chakazul/Lenia/Python/animals.json` (`O2u`, `OV2u`, `OG2g`, `S2v`); quad4 polynomial kernel per upstream `LeniaNDK.py` kernel_core[0]; first `common-py` rule-of-three promotion-review (documentation-only); two Blender Cycles hero scripts (`render_lenia_2d.py` + `render_lenia_3d.py`) |
...
### Runtime backend selection (Stack D)

For Stack D sims with multiple backend paths (e.g., FFT backends in lenia-fft), the sim's `pyproject.toml` declares each as an optional extra; the sim's init probes available backends in priority order via guarded import + smoke test; selected backend is logged at `log.info` (NOT warn — every run on alternate-hardware platforms would otherwise create noise); failures are non-fatal and fall through to the next priority. Universal baseline (Taichi-real-space or numpy) is always available; CuPy / PyTorch require optional pip extras gated on user hardware. CI exercises ONLY the universal-baseline backends — GPU-specific backends require user-hardware verification per the runtime-only-surface convention. Banked Phase 10; see `docs/conventions.md` "Runtime backend selection (Stack D)" and `continuous-ca/lenia-fft/python/lenia_fft/fft_backend.py` as the canonical example.

### Tier-label runtime mutation post-init

For sim-tier dropdowns whose interactivity depends on runtime conditions (FFT-backend availability, GPU device class), build the tier dropdown's labels at init time AFTER the relevant runtime probe. The tier-list data structure must be a mutable `list[tuple[...]]` (or similar), not a `Final[tuple[...]]`. The label string is the base + a runtime-determined suffix (`" (FFT)"` vs `" (real-space, capture-mode)"`). Diverges from Phase 9's compile-time-constant tier table. Banked Phase 10; see `continuous-ca/lenia-fft/python/lenia_fft/main.py` `TIERS_BASE` + post-backend-select mutation block.
...
- Stack D lenia-fft sim: `cd continuous-ca/lenia-fft/python && pip install -e . && python main.py` (universal baseline; optional GPU-FFT extras: `pip install -e .[cuda]` for NVIDIA / `pip install -e .[rocm]` for AMD)
