...
## Priority 1.12 — Diagnose actual cause of sustained fan load (was: VSync default + panel toggle — retracted)

The thermal observation during Phase 8 visual verification (RX 6800 XT fans maxed under sustained compute) was originally attributed to common-cpp's MAILBOX-preferred present mode in `choosePresentMode()` and a `Window` API amendment was banked as the fix. Direct test with `vblank_mode=3 ./build/bin/eulerian_smoke` (forces FIFO at the Mesa driver layer, equivalent to the proposed API amendment) **falsified the present-mode attribution** — fans stayed loud under FIFO. See `project-state.md` § 9 (retraction entry) and § 7 line 457(d) (architect-1 fabrication record) for the full provenance. Actual cause is uncharacterized; possible culprits include heavy-compute baseline for a 256³ solver on a 250 W card, a missing CPU-side frame-pacing cap in the render loop allowing compute scheduling to outrun presentation regardless of swapchain mode, or a specific hot solver kernel (Jacobi pressure at 40 iterations/frame is the prime suspect). v1.1 diagnostic work: capture `radeontop` / `nvtop` / a Vulkan profiler trace at the 256³ default tier with each preset, identify the dominant CPU/GPU time consumer, then decide whether the fix is a frame-pacing cap, a Jacobi-iteration count reduction, a default tier rebalance (cf. Priority 1.13), or something else. The `VSync panel toggle` UX previously specified here is on hold pending a real cause — adding a present-mode toggle for an effect that present mode doesn't control would be cargo-cult UX.
